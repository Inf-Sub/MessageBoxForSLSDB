...
class DBYFileViewer:
    ...
    def open_program(self, file_path):
        try:
            subprocess.Popen([self.executable_path, file_path], shell=True)  # Используем Popen чтобы не блокировать основной поток
            self.root.destroy()  # Закрываем окно после запуска программы
        except Exception as e:
            messagebox.showerror("Ошибка", f"Не удалось открыть файл: {str(e)}")
    ...
    def run(self):
        ...
        self.root = tk.Tk()  # Сохраняем ссылку на root в объекте класса
        self.root.title("DBY File Viewer")

        listbox = tk.Listbox(self.root, width=50, height=20)
